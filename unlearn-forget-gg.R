...
for(i in 1:nrow(dfmat)){
  ...
  #datavec = sampler(300)
  df[i,] = c(matrix(datavec[-c(1,2)],ncol=6)[c(4,5),])
}
...
# Create boxplots with Group2 on X-axis and Group3 distinguished by color

#tikz('ged_eps.tex',standAlone = TRUE, width = 8.7,height = 5)
ggsave(
  writefile,
  ggplot(longdf, aes(x = factor(eps), y = errs, fill = factor(noise))) +
    geom_boxplot(outlier.shape=NA, fatten = NULL) +
    labs(x = expression("Certifiability Parameter"~epsilon), y = "Error on unlearned set", fill = "noise") +
    ggtitle(bquote("UED vs."~epsilon *" for n = p = 1255 and m ="~ .(as.numeric(mval))))+
...
#tools::texi2dvi('ged_eps.tex',pdf=T)